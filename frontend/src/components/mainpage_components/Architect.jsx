...
const Architect = () => {
    ...
    return (
    <div className="flex flex-col  justify-around w-full min-h-[600px]  md:min-h-[800px] architect-bg-image bg-cover">
      {/* Header */}
      <div className="flex flex-col lg:flex-row h-[130px] md:h-[200px] lg:bg-black/55 pt-4 md:pt-0">

        {/* Header text */}
        <div className="relative bg-black/55 lg:bg-transparent flex-1 flex-col md:w-[75%] z-0">
          {/* Жёлтая полоска — ПОД текстом, НО НАД фоном */}
          <div className="bg-[#f9b33b] h-4 md:h-10 w-[90%] mx-auto absolute bottom-2 lg:bottom-6 md:bottom-8 left-0 right-0 md:left-[10px] z-[100]"></div>
        ...
        {/* Days */}
        <div className="flex flex-col items-center justify-center bg-[#64b1d9] text-black w-[50%] mx-auto lg:mx-0 md:w-[25%] lg:p-4 ">
          <p className="text-2xl sm:text-5xl md:text-7xl font-light hidden lg:inline">{t('left_block.p')}</p>
          <strong className="text-3xl sm:text-6xl md:text-7xl font-bold">{t('left_block.strong')}</strong>
        </div>
      </div>

      {/* Comment */}
      <div className="flex items-center bg-black/58 text-white text-[22px] sm:text-xl md:text-[29px] font-light px-4 py-5 md:px-32 md:py-0 md:h-[130px]">
      ...
      <div className="w-full  flex flex-col-reverse md:flex-row md:justify-around md:h-[270px] gap-6 lg:p-4 md:p-0">
        ...
        {/* Conditions */}
        <div className="md:w-[500px] bg-black/55  md:border-6 lg:border-[#f9b33b] lg:rounded-lg text-white px-4 md:px-5 md:py-10 lg:backdrop-blur-sm">
          <p className="font-semibold text-xl md:text-3xl hidden lg:inline">{t('condition.h')}</p>
          <ul className=" text-center font-[300] lg:font-[500] lg:text-start pl-6 pt-2 md:pt-1 text-base md:text-[22px]">
