...
const Discount = () => {
    ...
    return (
    <div className="flex flex-col  justify-around w-full min-h-[480px]   md:min-h-[800px] discount-bg-image bg-cover">
      {/* Header */}
      <div className="flex flex-row h-[80px] md:h-[200px] lg:bg-black/55 pt-4 md:pt-0">
        ...
        <div className="relative bg-black/55 lg:bg-transparent flex-1 md:w-[75%] z-0">
          ...
          {/* Текст — самый верхний */}
          <h1 className={`md:bg-transparent text-white text-center font-bold ${selectedLang === 'ru' ? 'text-[44px]' : 'text-[36px]'}  sm:text-2xl md:text-[140px] absolute inset-0 flex items-center justify-center z-[200] text-shadow-lg`}>
            {t('h')}
          </h1>
        </div>

        {/* Days */}
        <div className="flex flex-col items-center justify-center bg-[#ffb429] text-black w-[40%] md:w-[25%] p-4 md:p-5 lg:space-y-5 lg:mr-[45px]">
      ...
      <div className="w-full  flex flex-col-reverse md:flex-row md:justify-around md:h-[270px] gap-6 lg:p-4 md:p-0">
        ...
        <div className="md:w-[500px] bg-black/55  md:border-6 lg:border-[#f9b33b] lg:rounded-lg text-white px-4 md:px-5 md:py-7 lg:backdrop-blur-sm">
          ...
          <ul className=" text-center font-[200] lg:font-[500] lg:text-start pl-6 pt-2 md:pt-4 text-base md:text-[21px]">
            ...
            <li>{t('condition.li3')} <span className='text-[#ffb429]'>{t('condition.span3')}</span> {t('condition.li4')}</li>
          </ul>
        </div>
      </div>
    </div>
